...
export default class WScloudflare<ServerNameSpacesPaths extends string = string, ListenEvents extends EventsMap = DefaultEventMap, EmitEvents extends EventsMap = EventsMap, SocketData = any>  {
  ...
  constructor({ honoApp: honoAppConstructor }: { honoApp: Hono }) {
    WScloudflare.honoApp = honoAppConstructor;
    this.Sockets = <any>WScloudflare.sockets;
    this.Namespaces = <any>WScloudflare.namespaces;

    // Method for obtaining a namespace by path or creating a new one if it doesn't exist.
    this.of = (namespace) => {
      const hasNamespace = <any>WScloudflare.getNamespace(namespace);
  ...
  public static getNamespace(namespace: ServerNamespacesPath) {
    for (const item of WScloudflare.namespaces) {
  ...
  public static setupWsConnectionCloudflareWorker(request: HonoRequest, callback: (socket: Socket | any) => void, { namespace }: { namespace: NameSpace }, namespaceMiddlewares: NamespaceMiddleware) {
    ...
    WScloudflare.onSocketJoin(socket);
